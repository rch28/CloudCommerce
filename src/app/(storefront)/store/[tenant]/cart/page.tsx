      ...
      <div className="grid gap-8 lg:grid-cols-3">
          ...
          {items.map((item) => (
            <div key={item.variantId} className="flex gap-4 rounded-xl border border-border bg-card p-4">
              ...
              <div className="min-w-0 flex-1">
                <Link href={`${base}/products/${item.slug}`} className="font-medium text-foreground hover:text-[#8B5CF6] transition-colors">{item.productName}</Link>
                <p className="mt-0.5 text-sm text-[#7C3AED]">${item.price.toFixed(2)}</p>
                <div className="mt-2 flex items-center gap-3">
                  <div className="flex items-center rounded-lg border border-border">
                    <button onClick={() => updateQuantity(item.variantId, item.quantity - 1)} className="p-1.5 text-muted-foreground hover:text-foreground"><Minus size={12} /></button>
                    <span className="w-8 text-center text-sm text-foreground">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.variantId, item.quantity + 1)} className="p-1.5 text-muted-foreground hover:text-foreground"><Plus size={12} /></button>
                  </div>
        ...
        <div className="rounded-xl border border-border bg-card p-6 h-fit">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between border-t border-border pt-2 text-lg font-bold text-foreground"><span>Total</span><span>${pricing.total.toFixed(2)}</span></div>
          </div>
          ...
          <Link href={`${base}/products`} className="mt-3 flex w-full items-center justify-center text-sm text-muted-foreground hover:text-foreground transition-colors">
            Continue Shopping
          </Link>
        </div>
      </div>
