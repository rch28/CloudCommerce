...
export default function Topbar({ onMenu, onSearchOpen }: TopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center gap-4 border-b border-border bg-[#09090B]/80 px-4 backdrop-blur-xl sm:px-6">
      <button onClick={onMenu} className="shrink-0 text-muted-foreground transition-colors hover:text-[#F8FAFC] lg:hidden">
        <Menu size={20} />
      </button>

      <button
        onClick={onSearchOpen}
        className="hidden w-full max-w-md items-center gap-2 rounded-lg border border-border bg-card px-3.5 py-2 text-sm text-muted-foreground transition-colors hover:border-[#7C3AED]/30 sm:flex"
      >
      ...
      <div className="ml-auto flex items-center gap-2 sm:gap-3">
        ...
        {session ? (
          <div className="relative">
            <button
              onClick={() => setMenu((m) => !m)}
              className="flex items-center gap-2 rounded-lg border border-border bg-card py-1.5 pl-1.5 pr-2.5 transition-colors hover:border-[#7C3AED]/30"
            >
              <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#7C3AED] text-xs font-bold text-white">
                {initials}
              </div>
              <span className="hidden text-sm font-medium text-[#F8FAFC] sm:inline">{session.name}</span>
            ...
            {menu && (
              <>
                ...
                <div className="absolute right-0 z-20 mt-2 w-60 overflow-hidden rounded-xl border border-border bg-card shadow-2xl shadow-black/40">
